-- Controle de embarque manual por empresa + auditoria de origem da validação.

ALTER TABLE public.companies
  ADD COLUMN IF NOT EXISTS allow_manual_boarding boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.companies.allow_manual_boarding IS 'Quando false, validações manuais na lista do motorista ficam bloqueadas (somente QR scanner).';

ALTER TABLE public.ticket_validations
  ADD COLUMN IF NOT EXISTS validation_source text NOT NULL DEFAULT 'scanner';

ALTER TABLE public.ticket_validations
  DROP CONSTRAINT IF EXISTS ticket_validations_validation_source_check;

ALTER TABLE public.ticket_validations
  ADD CONSTRAINT ticket_validations_validation_source_check
  CHECK (validation_source IN ('scanner', 'manual_list'));
...
BEGIN
  ...
  IF v_ticket_id IS NULL THEN
    IF v_user_company_id IS NOT NULL THEN
      INSERT INTO public.ticket_validations (
        ...
        validation_source, validated_by_user_id, validated_at, device_info, app_version
      ) VALUES (
        v_user_company_id, v_action, 'blocked', 'invalid_qr',
        v_source, v_user_id, now(), p_device_info, p_app_version
      );
    END IF;
  ...
  INSERT INTO public.ticket_validations (
    ...
    validation_source, validated_by_user_id, validated_by_driver_id,
    ...
    v_source, v_user_id, v_driver_id, now(), p_device_info, p_app_version
  );
