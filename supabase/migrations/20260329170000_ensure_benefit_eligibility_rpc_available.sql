-- Garante que a RPC pública de elegibilidade de benefícios exista no ambiente (correção do 404 em /rpc/get_benefit_eligibility_matches).
...
declare
  ...
  v_ref_date date;
begin
  -- Comentário curto: normalização defensiva para manter compatibilidade com o frontend.
...
comment on function public.get_benefit_eligibility_matches(uuid, uuid, text, date) is
  'Retorna matches de elegibilidade de benefício por CPF para checkout público/admin.';
