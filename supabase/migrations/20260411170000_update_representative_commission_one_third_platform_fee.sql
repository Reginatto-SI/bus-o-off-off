-- Regra oficial 2026-04-11:
-- comissão do representante = 1/3 da taxa da plataforma da empresa da venda.
-- Arredondamento oficial: 2 casas para percentual e valor monetário.
...
BEGIN
  ...
  -- Comissão só nasce após venda efetivamente paga.
  ...
  -- Base operacional da comissão permanece a venda confirmada.
  v_base_amount := COALESCE(
    v_sale.gross_amount,
    (COALESCE(v_sale.unit_price, 0) * COALESCE(v_sale.quantity, 0))::numeric
  )::numeric(10,2);

  -- Arredondamento oficial em centavos.
  ...
  -- Regra existente preservada: wallet ausente não quebra confirmação; fica bloqueada.
...
COMMENT ON FUNCTION public.upsert_representative_commission_for_sale(uuid, text) IS
  'Cria comissão idempotente por sale_id após venda paga. Regra oficial: comissão percentual = ROUND(platform_fee_percent/3, 2) e valor arredondado em 2 casas.';

COMMENT ON COLUMN public.representatives.commission_percent IS
  'Campo mantido por compatibilidade histórica. A regra operacional de comissão do representante usa 1/3 da taxa da plataforma da venda.';
