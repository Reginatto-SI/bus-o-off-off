-- Correção controlada de passageiro em operação única (update + auditoria)
-- Objetivo: remover coordenação sensível do frontend e garantir consistência atômica no backend.
...
IS 'Correção transacional de passageiro (ticket + sale_logs) com validações de status e multiempresa.';

GRANT EXECUTE ON FUNCTION public.correct_sale_passenger(uuid, uuid, text, text, text, text)
TO authenticated;
