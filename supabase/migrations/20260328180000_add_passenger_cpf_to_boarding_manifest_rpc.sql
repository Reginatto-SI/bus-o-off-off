-- Inclui CPF mascarável no payload da lista de embarque para PDF e pré-visualização detalhada.
-- Observação: alteração de OUT parameters exige DROP prévio da função para evitar erro 42P13.
drop function if exists public.get_boarding_manifest_rows(uuid, uuid, uuid);
...
as $$
  select
    ...
    coalesce(t.passenger_cpf, s.customer_cpf) as passenger_cpf,
