CREATE OR REPLACE FUNCTION public.get_company_activation_report()
 RETURNS TABLE(id uuid, name text, legal_name text, trade_name text, cnpj text, document text, document_number text, email text, phone text, whatsapp text, city text, state text, is_active boolean, created_at timestamp with time zone, updated_at timestamp with time zone, asaas_account_id_production text, asaas_wallet_id_production text, asaas_onboarding_complete_production boolean, asaas_pix_ready_production boolean, asaas_pix_last_checked_at_production timestamp with time zone, asaas_account_id_sandbox text, asaas_wallet_id_sandbox text, asaas_onboarding_complete_sandbox boolean, asaas_pix_ready_sandbox boolean, asaas_pix_last_checked_at_sandbox timestamp with time zone, event_count bigint, sale_count bigint, paid_sale_count bigint, vehicle_count bigint, driver_count bigint, last_activity_at timestamp with time zone)
 LANGUAGE plpgsql
 SECURITY DEFINER
