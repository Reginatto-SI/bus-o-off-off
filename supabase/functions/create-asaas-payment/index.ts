...
import {
  ensureSaleTermsAcceptance,
  getPayloadTermsAcceptance,
} from "../_shared/sale-terms-acceptance.ts";
...
serve(async (req) => {
  ...
  try {
    ...
    // Guarda multi-gateway (aditiva): o gateway congelado na venda decide. Nunca
    // cobrar no Asaas uma venda vinculada a outro gateway.
    if (sale.payment_gateway && sale.payment_gateway !== "asaas") {
      return jsonResponse(
        { error: "Esta venda não usa Asaas.", error_code: "gateway_mismatch", sale_gateway: sale.payment_gateway },
        409,
      );
    }
    ...
    const { data: company, error: companyError } = await supabaseAdmin
      .from("companies")
      .select(
        "name, payment_environment, asaas_wallet_id_production, asaas_api_key_production, asaas_onboarding_complete_production, asaas_pix_ready_production, asaas_wallet_id_sandbox, asaas_api_key_sandbox, asaas_onboarding_complete_sandbox, asaas_pix_ready_sandbox, platform_fee_percent, socio_split_percent",
      )
